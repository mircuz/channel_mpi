...
dnsdata_113read_restart_file();  cont_=(int)floor(time_/dt_field_);
/*!outstats()*/
   fprintf(stdout,"""nx=""");fprintf(stdout,"%d",nx_);fprintf(stdout,"\t");fprintf(stdout,"""nxd=""");fprintf(stdout,"%d",nxd_);fprintf(stdout,"\t" );fprintf(stdout,"""nz=""");fprintf(stdout,"%d",nz_);fprintf(stdout,"\t");fprintf(stdout,"""nzd=""");fprintf(stdout,"%d",nzd_);putc('\n',stdout);

forward:; while( time_ <t_max_-deltat_/2.){
  time_+=2./dnsdata_117RK1_rai_coeff*deltat_;
  dnsdirect_8buildrhs(dnsdata_116RK1_rai); dnsdirect_9linsolve(dnsdata_117RK1_rai_coeff/deltat_);
  dnsdirect_10vetaTOuvw();  dnsdirect_13computeflowrate(dnsdata_117RK1_rai_coeff/deltat_);
  time_+=2./dnsdata_119RK2_rai_coeff*deltat_;
  dnsdirect_8buildrhs(dnsdata_118RK2_rai); dnsdirect_9linsolve(dnsdata_119RK2_rai_coeff/deltat_);
  dnsdirect_10vetaTOuvw();  dnsdirect_13computeflowrate(dnsdata_119RK2_rai_coeff/deltat_);
  time_+=2./dnsdata_121RK3_rai_coeff*deltat_;
  dnsdirect_8buildrhs(dnsdata_120RK3_rai); dnsdirect_9linsolve(dnsdata_121RK3_rai_coeff/deltat_);
  dnsdirect_10vetaTOuvw();  dnsdirect_13computeflowrate(dnsdata_121RK3_rai_coeff/deltat_);
  dnsdata_112outstats();
  if( reread_ ){ dnsdata_1read_initial_data(); reread_=0;};
};forward_exit:;
freemem(es);return 0;}
